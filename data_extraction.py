...
with open('phishing_urls_fixed_new.csv','r') as csvfile:

    data_set = []
    csvreader = csv.reader(csvfile)
    for cnt, row in enumerate(csvreader):

        print('Item '+str(cnt+1))
        print('\n')
        ...
        if response == "":
            ...
            try:
                cre_date = whois_response.creation_date
                if isinstance(cre_date,int):
                    cre_date = whois_response.creation_date[0]
                else:
                    cre_date = whois_response.creation_date

                if diff_month(date.today(), cre_date) >= 6:
                    data_row.append(-1)
                    print('23 OK')
        ...
        outfile = open('training_dataset.csv', 'w', newline='')
        out = csv.writer(outfile)
        out.writerows(data_set)
        outfile.close()
